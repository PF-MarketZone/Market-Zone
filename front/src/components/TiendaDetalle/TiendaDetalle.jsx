...
import { setCategoriaFiltro, setPrecioMinFiltro, setPrecioMaxFiltro } from "../../redux/actions";
...
const TiendaDetalle = () => {
  ...
  const minPriceValue = tienda.products.reduce((min, producto) => Math.min(min, producto.price), 0);
  const maxPriceValue = tienda.products.reduce((max, producto) => Math.max(max, producto.price), 0);

  const filteredProductos = tienda.products
    .filter(producto => !selectedCategory || producto.categories.includes(selectedCategory))
    .filter(producto => producto.price >= priceRange[0] && producto.price <= priceRange[1]);
    ...
    <div className={styles.tiendaDetalleContainer}>
      <div className={styles.filtrosContainer}>
        ...
        >
          ...
          {Array.from(new Set(tienda.products.flatMap(producto => producto.categories))).map(category => (
